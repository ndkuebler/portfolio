...
export default function HomePage() {
  ...
  /* 
    ✅ ONCE PER SESSION
    - Lightning ALWAYS renders on first paint (so SVG animations are correct)
    - If intro already played, we immediately turn it off AFTER mount
    - Carousel + timing untouched
  */
  useEffect(() => {
    const hasPlayed = sessionStorage.getItem("nk_intro_played");

    if (hasPlayed) {
      // kill intro immediately on return nav
      setShowIntro(false);
    } else {
    ...
    <main className="relative min-h-screen bg-black overflow-hidden text-white">
            ...
            <div className="nk-track">
              {[1, 2].map((_, i) => (
                <div key={i} className="flex gap-12">
                  ...
                  <a href="/work/watershield-figma" className="nk-item wide">
      ...
      <style jsx>{`
        .nk-track {
          display: flex;
          width: max-content;
          animation: nk-scroll 15s linear infinite;
          padding-right: 3rem;
        }
